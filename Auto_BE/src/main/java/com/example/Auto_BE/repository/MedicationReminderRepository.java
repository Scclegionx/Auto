...
public interface MedicationReminderRepository extends JpaRepository<MedicationReminder, Long> {
    ...
    // Query để lấy reminder với eager loading elderUser và deviceTokens cho cron job
    @Query("SELECT m FROM MedicationReminder m " +
            "JOIN FETCH m.elderUser u " +
    ...
    // Query để lấy tất cả thuốc cùng thời gian của elderUser với eager loading
    @Query("SELECT DISTINCT m FROM MedicationReminder m " +
            "LEFT JOIN FETCH m.elderUser u " +
            "WHERE u.id = :elderUserId " +
            "AND m.reminderTime = :reminderTime " +
            "AND m.isActive = true")
    List<MedicationReminder> findByUserIdAndReminderTime(@org.springframework.data.repository.query.Param("elderUserId") Long elderUserId, 
    ...
    @Query("SELECT m FROM MedicationReminder m " +
            "WHERE m.elderUser.id = :elderUserId " +
            "AND m.reminderTime = :reminderTime " +
            "AND m.isActive = true")
    List<MedicationReminder> findByUserIdAndReminderTimeSimple(@org.springframework.data.repository.query.Param("elderUserId") Long elderUserId, 
                                                              @org.springframework.data.repository.query.Param("reminderTime") String reminderTime);
}