...
class GPUTrainer:
    ...
    def load_data(self, file_path: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        if not os.path.exists(file_path):
            self.logger.error(f"ERROR Data file not found: {file_path}")
        ...
        try:
            ...
            self.logger.error(f"ERROR Invalid JSON in data file: {file_path}")
            raise ValueError(f"Invalid JSON in data file: {file_path}")
        except UnicodeDecodeError:
            self.logger.error(f"ERROR Encoding issue in data file: {file_path}")
            # Try with different encodings
            try:
                with open(file_path, 'r', encoding='latin-1') as f:
                    data = json.load(f)
                self.logger.warning("WARNING Had to use latin-1 encoding for data file")
            except:
                raise ValueError(f"Unable to decode data file: {file_path}")
        
        if not data:
            self.logger.error("ERROR Empty dataset loaded")
            raise ValueError("Empty dataset loaded")
            
        # Handle unique dataset format (train/val split)
        if isinstance(data, dict) and 'train' in data and 'val' in data:
            self.logger.info(f"Using pre-split dataset: Train={len(data['train'])}, Val={len(data['val'])}")
            # Use train data for training, val data for validation
            train_data = data['train']
            val_data = data['val']
            self.logger.info(f"Combined dataset: {len(train_data)} train + {len(val_data)} val samples")
        else:
            # Regular dataset format - split manually with stratify
            from sklearn.model_selection import train_test_split
            # Extract labels for stratify
            ...
            self.logger.info(f"📈 Loaded {len(data)} samples, split into {len(train_data)} train + {len(val_data)} val")
        ...
        if self.enable_multi_task:
            # Handle dataset structure (dict with train/val keys or list)
...
def main():
    ...
    try:
        ...
        # Load data first to get actual label counts
        # Ưu tiên dataset đã clean entity labels để tránh CUDA errors
        # Get absolute path to dataset
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(script_dir)))
        dataset_file = os.path.join(project_root, "src", "data", "raw", "elderly_command_dataset_FULL_13C_FIXED.json")
        ...
        # Load dataset to get actual label counts - với memory optimization
        import json
        print("Loading dataset...")
        with open(dataset_file, 'r', encoding='utf-8') as f:
            actual_data = json.load(f)
        
        # Sử dụng toàn bộ dataset cho GPU training
        print(f"Using full dataset: {len(actual_data)} samples for GPU training")
